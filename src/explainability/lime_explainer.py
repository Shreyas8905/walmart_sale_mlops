...
def generate_lime_explanations(
    ...
    if LimeTabularExplainer is not None:
        ...
        LOGGER.warning(
            "lime package unavailable; generating deterministic fallback local explanations"
        )

    output_paths: list[Path] = []
    for sample_number, sample_index in enumerate(sample_indices[:5]):
        html_path = (
            settings.paths.reports / f"lime_explanation_sample_{sample_number}.html"
        )
        png_path = (
            settings.paths.reports / f"lime_explanation_sample_{sample_number}.png"
        )
