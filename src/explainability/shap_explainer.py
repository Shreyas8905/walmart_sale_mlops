...
def generate_shap_explanations(
	...
	if shap is not None:
		try:
			if hasattr(model, "estimators_") or model.__class__.__name__.lower().startswith(("randomforest", "gradientboosting", "xgb", "histgradientboosting")):
				explainer = shap.TreeExplainer(model)
			elif hasattr(model, "coef_"):
				explainer = shap.LinearExplainer(model, transformed, feature_perturbation="interventional")
			else:
				background = shap.sample(transformed, min(100, len(transformed)), random_state=settings.project.random_seed)
		...
		LOGGER.warning("shap package unavailable; using deterministic fallback contribution approximation")
	...
	plt.xticks(range(len(top)), [str(feature_names[index]) for index in top], rotation=45, ha="right")
	...
	ranked_indices = np.argsort(np.abs(sample_values))[::-1][: min(12, len(sample_values))]
	plt.figure(figsize=(10, 6))
	plt.barh(
