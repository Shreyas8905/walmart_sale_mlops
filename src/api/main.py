...
async def add_request_context(request: Request, call_next):
	...
	try:
		...
		LOGGER.exception("Unhandled request error request_id=%s", request_id, extra={"request_id": request_id})
		return JSONResponse(
			status_code=500,
			content={"error": "internal_server_error", "request_id": request_id, "detail": str(exc)},
		)
