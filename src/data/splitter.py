...
def temporal_split(frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.Series, pd.Series, pd.Series]:
	...
	sorted_frame = frame_with_date.sort_values("__parsed_date").drop(columns=["__parsed_date"]).reset_index(drop=True)
